...
import edu.wm.cs.cs301.danielquiroga.generation.BSPBranch;
import edu.wm.cs.cs301.danielquiroga.generation.BSPLeaf;
import edu.wm.cs.cs301.danielquiroga.generation.BSPNode;
...
import edu.wm.cs.cs301.danielquiroga.generation.MazeContainer;
import edu.wm.cs.cs301.danielquiroga.generation.Seg;
...
public class MazeFileReader {
	 ...
	 *  
	 * @param file with data of a Maze object
	 */
	public MazeFileReader(FileInputStream file) {
		load(file) ;
	}
	 ...
	 * @param file gives the input file
	 */
		...
		try{
			...
			for (int temp = 0 ; temp < nList.getLength() ; temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					Log.v("reader rooms ", String.valueOf(rooms));
	...
	private static Seg readSegment(Element eElement, int number, int i) {
		...
		//Log.v("MazeFilerReader", String.valueOf(color.getComponents()[0]));
		com = MazePanel.getColorComponents(col);
		//com[0] = Color.red(col);//(int) color.getComponents()[0];// TODO try to fix it so it calls on mazePanel
